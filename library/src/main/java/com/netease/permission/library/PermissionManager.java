...
public class PermissionManager {
     ...
     * 这个方法 invoke后 就到MainActivity中的locationContactsTask 这个方法。
     *  locationContactsTask没有入参的话，invoke就一个入参的话 就正常 ；加入参数就不正常报错
     * @param activity    当前Activity
        ...
        for (Method method : methods) {
            ...
            if (method.isAnnotationPresent(IPermission.class)) {
                ...
                if (iPermission.value() == requestCode) {
                    ...
                    Class<?>[] s = new Class[]{View.class,int.class};
                    //TODO
...
//                    }
                    ...
                    try {
                        ...
                        Log.d("ws","requestCode:"+requestCode) ;

                        method.invoke(activity,requestCode);
                    } catch (Exception  e) {
                        Log.d("zjp", "reflectAnnotationMethod: " + e
                        .getLocalizedMessage() + " " + e.getMessage() + " " + e.toString());
                        e.printStackTrace();
                    }
                }
            }
    ...
    private void reflect(Activity act, View view) {
        try {
            Class<?> clazz = Class.forName("com.netease.permission.reflection.MainActivity");
            Method method = clazz.getMethod("locationContactsTask", View.class , int.class);
            method.setAccessible(true);
            Object invoke = method.invoke(act, view);   //
